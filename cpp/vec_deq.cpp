...
template<class T> class vec_deq {
  ...
  public:
  vec_deq(int _capacity) : capacity(_capacity) { 
    deq.resize(capacity << 1 | 1);
    pl = deq.begin() + _capacity;
    pr = deq.begin() + _capacity;
  }
  ...
  void pop_back() { --pr; assert(size() >= 0 && "cannot pop empty vec_deq"); }
  void pop_front() { ++pl; assert(size() >= 0 && "cannot pop empty vec_deq"); }
